...
// The WBModelFunc type is an adapter to allow the use of ordinary
// function as WBModel mutator.
type WBModelFunc func(context.Context, *ent.WBModelMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f WBModelFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.WBModelMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.WBModelMutation", m)
	}
	return f(ctx, mv)
}
